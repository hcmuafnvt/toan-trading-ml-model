...
"""
FX Coding — Stage 5_4 : Hybrid Overlay + ML Confidence
------------------------------------------------------
Mục tiêu: kết hợp model core alpha (T5_core) với overlay executor
(session filter + ATR TP/SL + probability threshold) để chuyển
statistical edge thành trading edge qua nhiều regime.

Input:
  - data/GBP_USD_M5_2024.parquet           (OHLC mid from OANDA)
  - logs/stage2_features.csv               (X, stride=5, window=200)
  - logs/stage3_y.csv                      (y, [0,1,2] – optional, không dùng ở đây)
  - logs/T5_core_alpha_lightgbm.txt        (LightGBM multiclass 3 lớp)

Output:
  - logs/stage5_4_results.csv
  - logs/stage5_4_summary.txt
"""
...
MODEL_FILE = "logs/T5_core_alpha_lightgbm.txt"  # model từ Stage 4B

# feature sampling params (giữ đúng Stage 2/3)
WINDOW = 200
STRIDE = 5
...
# overlay params
SESSIONS_TO_TRADE = ("London", "NewYork")              # session filter
PROB_THRESH_GRID  = [0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90]
ATR_MULT_GRID     = [1.5, 2.0]                         # TP/SL = ATR * k  (RR 1:1)
MIN_BARS_AFTER_ENTRY = 0                               # để đơn giản: 0 (có thể tăng sau)
...
def detect_session_from_hour(h):
    # UTC sessions (giữ đúng Stage 1 logic)
    if 7 <= h < 15:
        return "London"
    elif 12 <= h < 21:
        return "NewYork"
...
def atr_series(price_df: pd.DataFrame, window: int = 14) -> pd.Series:
    # ta.volatility làm việc tốt với Series thẳng (đã flatten)
    atr = AverageTrueRange(
        price_df["high"], price_df["low"], price_df["close"], window=window
    ).average_true_range()
...
if __name__ == "__main__":
    ...
    # 2) Load features & align sample times (giữ đúng WINDOW/STRIDE)
    print(f"⏳ Loading features: {FEATURE_FILE}")
    X_full = pd.read_csv(FEATURE_FILE)
    # sanitize columns để khớp với tên trong model
    X_full.columns = (
        X_full.columns
    ...
    probs = booster.predict(Xsub.values)  # shape [n_samples, 3]
    probs = np.asarray(probs)
    pred_cls = probs.argmax(axis=1)
    max_prob = probs.max(axis=1)

    # 4) Map signal to full timeline & prepare overlay masks
    signal_base = pd.Series(pred_cls, index=sample_times).reindex(
        price.index, method="ffill"
    ).fillna(1).astype(int)  # default TIMEOUT
    prob_base = pd.Series(max_prob, index=sample_times).reindex(
        price.index, method="ffill"
    ).fillna(0.0)

    session_mask = price["session"].isin(SESSIONS_TO_TRADE)

    # 5) Grid search prob threshold & ATR mult
    rows = []
    print("\n========== Stage 5_4 Hybrid Overlay + ML Confidence ==========")
    print(f"Sessions traded: {SESSIONS_TO_TRADE}")
    for th in PROB_THRESH_GRID:
        tradable = session_mask & (prob_base >= th)
        sig = signal_base.where(tradable, 1)  # non-tradable → TIMEOUT
        ...
        for k in ATR_MULT_GRID:
            ...
            print(f"Trades={trades} | Win%={winrate:.2f} | PF={pfactor:.2f} | "
    ...
    res = pd.DataFrame(rows).sort_values(
        ["profit_factor", "expectancy_pips", "winrate_%"],
        ascending=False
    )
    ...
    with open(OUT_TXT, "w") as f:
        ...
        f.write("\n\nBest rows where PF>=1 and trades>=50:\n")
